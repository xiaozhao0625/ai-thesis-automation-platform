...
if (-not $TargetArtifactDirectory) { $TargetArtifactDirectory = Join-Path $PlatformRoot '.runtime\restore-artifact-store' }
$ArtifactRoot = [System.IO.Path]::GetFullPath($TargetArtifactDirectory)
...
$PreviousArtifactHostPath = $env:ARTIFACT_STORE_HOST_PATH
$env:ARTIFACT_STORE_HOST_PATH = $ArtifactRoot
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Copying database dump into PostgreSQL failed' }
    ...
    if ($null -eq $PreviousArtifactHostPath) { Remove-Item Env:ARTIFACT_STORE_HOST_PATH -ErrorAction SilentlyContinue } else { $env:ARTIFACT_STORE_HOST_PATH = $PreviousArtifactHostPath }
}
...
Write-Host "Restored Artifact Store: $ArtifactRoot"
