...
默认目标数据库为 `thesis_platform_restored`，默认 Artifact 目标为 `platform/.runtime/restore-artifact-store`。它们都与正在运行的正式数据库和 `platform/artifact_store` 隔离，因此可在同一台验收机演练恢复而不覆盖现有数据。也可显式指定：

```powershell
platform/scripts/restore-data.ps1 `
  -BundleDirectory <handoff-bundle> `
  -TargetDatabase thesis_platform_restore_check `
  -TargetArtifactDirectory D:\p1-1-restore-check\artifact-store
```
