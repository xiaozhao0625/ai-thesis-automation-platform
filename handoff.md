...
- `90_可恢复归档/20260715_历史工作文件/`：已保存旧版 UI/UX 构建脚本、渲染输出和原型工作文件，并附有归档说明。

此次整理没有删除原始资料、论文模板、研发基线或 Git 源码。旧 `work` 工作文件已迁入可恢复归档，`work/__pycache__` 作为可自动重建的缓存已删除。隔离 worktree 已完成合并并从 Git 工作树注册表移除；如其物理残留目录仍被 Windows 占用，可在关闭占用进程后手工删除，不影响 main 中的已提交内容。
