...
def build_docx(path: Path, project_text: str | None = None) -> None:
    project_text = project_text or (
        "项目约束：主控必须使用 STM32F103C8T6；温湿度模块采用 DHT11；"
        "无线模块采用 ESP8266-01S；显示屏为 0.96 英寸 OLED，驱动芯片 SSD1306；"
        "RTC模块采用 DS3231；供电电压为 3.3V。"
    )
    document = f"""<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    <w:p><w:pPr><w:pStyle w:val="Heading1"/></w:pPr><w:r><w:t>硬件选型</w:t></w:r></w:p>
    <w:p><w:r><w:t>{project_text}</w:t></w:r></w:p>
...
def main() -> None:
    ...
    generic_docx = FIXTURES / "generic" / "task-document" / "通用型号任务书.docx"
    ...
    build_docx(
        generic_docx,
        "主控采用 RP2040，温湿度传感器采用 HDC1080，蓝牙模块采用 HC-05，"
        "显示模块采用 LCD1602，RTC模块采用 DS3231。",
    )
    build_xlsx(xlsx)
    ...
    expected = {
        ...
        "required_values": ["STM32F103C8T6", "DHT11", "ESP8266-01S", "SSD1306", "DS3231", "FastAPI", "PostgreSQL", "3.3V"],
