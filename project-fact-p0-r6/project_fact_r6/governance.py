...
from .extractor import extract_fixture_set, find_facts
...
RETRIEVAL_REQUEST_SCHEMA = json.loads((SCHEMA_DIR / "retrieval-request.schema.json").read_text(encoding="utf-8"))
...
NON_MODEL_TECHNICAL_TOKEN = re.compile(r"^(?:PYTHON|WIFI|ISO|UART|HTTP)\d+$", re.I)
AUTHORIZED_APPROVERS = {"operator-lin-chen", "reviewer-zhang"}
SERVER_OWNED_RETRIEVAL_FIELDS = {
    "locked_model", "fact_version_id", "match_type", "supports_project_fact",
    "supports_model_parameters", "alias_scope",
}
NODE_CONTEXT_REGISTRY = {
    "outline_plan": "PROJECT_FACT_OUTPUT",
    "section_body": "PROJECT_IMPLEMENTATION",
    "bom_generate": "PROJECT_FACT_OUTPUT",
    "parameter_table_generate": "PROJECT_FACT_OUTPUT",
    "figure_generate": "PROJECT_FACT_OUTPUT",
    "test_chapter_generate": "PROJECT_IMPLEMENTATION",
    "comparison_section": "COMPARISON_ONLY",
    "background_section": "BACKGROUND_ONLY",
}


def _current_fact_context(
    fact_id: str,
    *,
    ...
    if active_snapshot is None or active_snapshot.get("status") != "ACTIVE":
        raise ValueError("Retrieval requires the current ACTIVE ProjectFactSnapshot")
    snapshot_refs = [item for item in active_snapshot.get("facts", []) if item.get("fact_id") == fact_id]
    if len(snapshot_refs) != 1:
        raise ValueError("Retrieval fact_id must be referenced exactly once by the current ACTIVE ProjectFactSnapshot")
    snapshot_ref = snapshot_refs[0]

    facts_by_id = {item.get("fact_id"): item for item in facts or []}
    fact = facts_by_id.get(fact_id)
    if not fact or fact.get("status") != "LOCKED":
        raise ValueError("Retrieval requires a current LOCKED ProjectFact")
    fact_version_id = snapshot_ref.get("fact_version_id")
    if fact.get("current_fact_version_id") != fact_version_id:
        raise ValueError("ACTIVE Snapshot does not reference the current ProjectFactVersion")

    versions_by_id = {item.get("fact_version_id"): item for item in fact_versions or []}
    version = versions_by_id.get(fact_version_id)
    if not version or version.get("fact_id") != fact_id or version.get("status") != "LOCKED":
        raise ValueError("ACTIVE Snapshot must reference a LOCKED ProjectFactVersion")
    if snapshot_ref.get("canonical_value") != version.get("canonical_value"):
        raise ValueError("ACTIVE Snapshot canonical value does not match its ProjectFactVersion")
    return fact, version, snapshot_ref
...
def _require_current_alias(
    ...
    *,
    ...
    fact_version_id = current_version["fact_version_id"]
    ...
    if current_version.get("fact_id") != fact_id or current_version.get("status") != "LOCKED":
        raise ValueError("Confirmed alias must reference a LOCKED ProjectFactVersion")
    if current_fact.get("status") != "LOCKED" or current_fact.get("current_fact_version_id") != fact_version_id:
        raise ValueError("Confirmed alias must reference the current locked ProjectFact")
    if snapshot_ref.get("fact_version_id") != fact_version_id:
...
def validate_retrieval_classification(
    ...
    *,
    ...
    if not isinstance(fact_id, str) or not fact_id:
        raise ValueError("Retrieval classification requires fact_id")
    submitted_server_fields = sorted(SERVER_OWNED_RETRIEVAL_FIELDS & set(record))
    if submitted_server_fields:
        raise ValueError("Retrieval request contains server-owned fields: " + ", ".join(submitted_server_fields))
    request_errors = validate_draft_202012_instance(record, RETRIEVAL_REQUEST_SCHEMA, "RetrievalRequest")
    if request_errors:
        raise ValueError("Retrieval request schema validation failed: " + "; ".join(request_errors))
    current_fact, current_version, snapshot_ref = _current_fact_context(
        fact_id,
    ...
    locked_model = current_version["canonical_value"]
    ...
    if match_type == "EXACT_MODEL":
        if record.get("alias_id") is not None:
        ...
        alias_scope = verified_alias["scope"]
    ...
    normalized = {
        ...
        "alias_id": verified_alias["alias_id"] if verified_alias else None,
        "supports_project_fact": expected[0],
        "supports_model_parameters": expected[1],
    }
    if verified_alias:
        normalized["alias_scope"] = verified_alias["scope"]
    return normalized
...
def protected_models_from_snapshot(snapshot: dict[str, Any], entities: dict[str, Any]) -> dict[str, dict[str, str]]:
    if snapshot.get("status") != "ACTIVE":
        raise ValueError("ProjectFact bindings require an ACTIVE Snapshot")
    protected: dict[str, dict[str, str]] = {}
    for fact_ref in snapshot["facts"]:
        fact, version, current_ref = _current_fact_context(
            fact_ref["fact_id"],
            fact_versions=entities["fact_versions"],
            facts=entities["facts"],
            active_snapshot=snapshot,
        )
        if fact["fact_type"] not in {"HARDWARE_MODEL", "MODULE_MODEL"}:
            continue
        protected[fact["fact_key"]] = {
            "fact_id": fact["fact_id"],
            "fact_version_id": version["fact_version_id"],
            "canonical_value": current_ref["canonical_value"],
        }
    return protected
...
def validate_generated_surfaces(snapshot: dict[str, Any], entities: dict[str, Any], surfaces: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if snapshot.get("status") != "ACTIVE":
        return [{
            "issue_code": "FACT_SNAPSHOT_NOT_ACTIVE",
            "severity": "BLOCKING",
    ...
    for surface in surfaces:
        node_definition_id = surface.get("node_definition_id")
        context_role = NODE_CONTEXT_REGISTRY.get(node_definition_id)
        if context_role is None:
            issues.append({
                "issue_code": "UNAUTHORIZED_CONTENT_CONTEXT",
                "severity": "BLOCKING",
                "surface_id": surface.get("surface_id"),
                "node_definition_id": node_definition_id,
            })
            continue
        submitted_context = surface.get("context_role") or surface.get("resolved_context_role")
        if submitted_context is not None and submitted_context != context_role:
            issues.append({
                "issue_code": "CONTEXT_ROLE_MISMATCH",
                "severity": "BLOCKING",
                "surface_id": surface.get("surface_id"),
                "submitted_context_role": submitted_context,
                "resolved_context_role": context_role,
            })
        ...
        if context_role in PROJECT_CONTEXTS:
            if not isinstance(bindings, dict):
                bindings = {}
            for fact_key, expected_binding in protected_models.items():
                matched = bindings.get(fact_key)
                if isinstance(matched, dict) and matched == expected_binding:
                    continue
                issues.append({
                    ...
                    "surface_id": surface.get("surface_id"),
                    "fact_key": fact_key,
                    "locked_model": expected_binding["canonical_value"],
                    "found_model": matched.get("canonical_value") if isinstance(matched, dict) else matched,
                    "match_type": "MISSING_BINDING" if matched is None else "STALE_OR_CONFLICTING_BINDING",
                })
            for fact_key in sorted(set(bindings) - set(protected_models)):
                issues.append({
                    "issue_code": "UNKNOWN_FACT_BINDING",
                    "severity": "BLOCKING",
                    "surface_id": surface.get("surface_id"),
                    "fact_key": fact_key,
                })
            ...
            for fact_key, fact_type, found_value, _start, _end in find_facts(content):
                if fact_type not in {"HARDWARE_MODEL", "MODULE_MODEL"}:
                    continue
                labelled_values.add(found_value.upper())
                expected_binding = protected_models.get(fact_key)
                if expected_binding and found_value.upper() != expected_binding["canonical_value"].upper():
                    issues.append({
                        "issue_code": "FACT_CONSTRAINT_VIOLATION",
                        "severity": "BLOCKING",
                        "surface_id": surface.get("surface_id"),
                        "fact_key": fact_key,
                        "locked_model": expected_binding["canonical_value"],
                        "found_model": found_value,
                        "match_type": classify_model(expected_binding["canonical_value"], found_value),
                    })
            bound_values = {item["canonical_value"].upper() for item in protected_models.values()}
            for token in {match.upper() for match in MODEL_LIKE_TOKEN.findall(content)}:
                if token in bound_values or token in labelled_values or NON_MODEL_TECHNICAL_TOKEN.fullmatch(token):
                    continue
                issues.append({
                    "issue_code": "UNCLASSIFIED_MODEL_TOKEN",
...
def project_bound_outputs(snapshot: dict[str, Any], entities: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    output_specs = [
        ("outline", "OUTLINE", "outline_plan"),
        ("body", "SECTION_BODY", "section_body"),
        ("bom", "BOM", "bom_generate"),
        ("parameter-table", "PARAMETER_TABLE", "parameter_table_generate"),
        ("figure", "FIGURE", "figure_generate"),
        ("test-chapter", "TEST_CHAPTER", "test_chapter_generate"),
    ]
    return [
        {
            ...
            "node_definition_id": node_definition_id,
            "resolved_context_role": NODE_CONTEXT_REGISTRY[node_definition_id],
            "content": "本输出受当前 ProjectFactSnapshot 的结构化事实绑定约束。",
            "fact_bindings": deepcopy(bindings),
        }
        for surface_id, surface_type, node_definition_id in output_specs
    ]
...
def build_review_payload(fixtures: Path) -> dict[str, Any]:
    ...
    return {
        "candidate": {"baseline": "v0.3.2-P0-r6", "prototype": "v1.2.4-P0-r6"},
        "initial": {
