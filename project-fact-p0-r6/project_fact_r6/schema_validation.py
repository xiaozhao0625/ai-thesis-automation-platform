    ...
    if isinstance(instance, dict):
        ...
        if additional is False:
            ...
            for key in sorted(set(instance) - set(properties)):
                errors.extend(validate_draft_202012_instance(instance[key], additional, f"{path}.{key}"))
        property_names = schema.get("propertyNames")
        if property_names:
            for key in instance:
                errors.extend(validate_draft_202012_instance(key, property_names, f"{path}.<property:{key}>"))
...
def validate_review_payload_instances(payload: dict[str, Any], schema_dir: Path) -> list[str]:
    ...
    for phase_name, phase in phases.items():
        ...
        for index, output in enumerate(phase.get("generated_outputs", [])):
            errors.extend(validate_draft_202012_instance(output, schemas["fact-bound-output.schema.json"], f"{phase_name}.generated_outputs[{index}]"))
        if phase.get("human_approval"):
            errors.extend(validate_draft_202012_instance(phase["human_approval"], schemas["human-approval.schema.json"], f"{phase_name}.human_approval"))
        if phase.get("audit_event"):
            errors.extend(validate_draft_202012_instance(phase["audit_event"], schemas["audit-event.schema.json"], f"{phase_name}.audit_event"))
