...
def validate_retrieval_classification(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if match_type == "CONFIRMED_ALIAS" and evidence_role == "MODEL_PARAMETER_EVIDENCE":
        if record.get("alias_scope") != "PROJECT_FACT_AND_PARAMETERS":
            raise ValueError("Confirmed alias cannot support model parameters outside its approved scope")
    return record
...
def validate_generated_surfaces(locked_model: str, surfaces: list[dict[str, str]]) -> list[dict[str, str]]:
    issues = []
    for surface in surfaces:
        for matched in re.findall(r"\bSTM32[A-Z0-9-]+\b", surface["text"], flags=re.I):
...
def build_entities(observations: list[dict[str, Any]], *, version: int = 2) -> dict[str, Any]:
    ...
    for fact_key, items in sorted(grouped.items()):
        ...
        fact = {
            ...
            "current_fact_version_id": f"{fact_id}-v{version}" if selected else None,
            "status": "LOCKED" if selected else "CONFLICT",
            "conflict_status": "NONE" if selected else "OPEN",
        }
        facts.append(fact)
        if selected:
            versions.append(make_version(fact_id, fact_key, selected, links, version))
            continue
        candidates = [
            {
...
def build_review_payload(fixtures: Path) -> dict[str, Any]:
    ...
    initial_snapshot = make_snapshot(initial_entities, 5)
    mcu_fact = next(item for item in initial_entities["facts"] if item["fact_key"] == "mcu_model")
    mcu_version = next(item for item in initial_entities["fact_versions"] if item["fact_version_id"] == mcu_fact["current_fact_version_id"])
    ...
    confirmation = resolve_conflict(initial_entities, initial_snapshot, conflict_entities, propagated_targets)
    ...
    return {
        "candidate": {"baseline": "v0.3.2-P0-r2", "prototype": "v1.2.4-P0-r2"},
        "initial": {
            ...
            "workflow_state": workflow_state(initial_targets, phase="initial"),
            "retrieval": retrieval,
        },
        "conflict": {
